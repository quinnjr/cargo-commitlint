...
impl HookInstaller {
    ...
    fn generate_hook_script(binary_path: &Path) -> String {
        // Single quotes are the only POSIX sh construct that neutralises every
        // metacharacter ($, backticks, "), so a checkout path containing them
        // cannot execute on commit. A literal ' becomes '\\''.
        let path_str = format!("'{}'", binary_path.to_string_lossy().replace('\'', r"'\''"));
...
mod shell_quote_tests {
    ...
    /// A checkout path containing shell metacharacters must not execute on commit.
    #[test]
    fn generated_script_quotes_path_literally() {
        let script = HookInstaller::generate_hook_script(Path::new(
            "/tmp/$(touch pwned)/it's/cargo-commitlint",
        ));
        // the path is single-quoted, with the embedded quote escaped as '\''
        assert!(script.contains(r"'/tmp/$(touch pwned)/it'\''s/cargo-commitlint'"));
        // and never appears wrapped in double quotes, which would allow substitution
        assert!(!script.contains(r#""/tmp/$(touch pwned)"#));
    }
}
