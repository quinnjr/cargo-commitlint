...
impl ValidationResult {
    ...
    pub fn format(&self, color: bool, verbose: bool) -> String {
        ...
        if self.valid && !verbose {
            ...
            let problems: Vec<&RuleResult> = self.results.iter().filter(|r| !r.valid).collect();
            ...
            for result in &problems {
                ...
                if color {
                    ...
                    output.push_str(&format!(
                        "{}   {} [{}] [{}]\n",
                        symbol, result.message, level_str, result.name
                    ));
                }
            }
...
impl Validator {
    ...
    // Results are pushed in rule-category groups rather than built as one
    // `vec![]` literal, so each block stays readable and greppable.
    ...
    pub fn validate(&self, message: &str) -> ValidationResult {
        ...
        let errors = active_results
            .iter()
            .filter(|r| !r.valid && r.level == RuleLevel::Error)
            .count();
        let warnings = active_results
            .iter()
            .filter(|r| !r.valid && r.level == RuleLevel::Warning)
            .count();
    ...
    fn validate_body_max_line_length(&self, commit: &ConventionalCommit) -> RuleResult {
        ...
        if result {
            ...
            let msg = format!(
                "body line {} must not be longer than {} characters (found {})",
                line_num, max, longest_line
            );
            match rule.level {
    ...
    fn validate_signed_off_by(&self, commit: &ConventionalCommit) -> RuleResult {
        ...
        let has_signoff = commit
            .trailers
            .keys()
            .any(|k| k.starts_with("Signed-off-by"));
        ...
        if result {
            ...
            let msg = match rule.applicable {
                ...
                Applicable::Never => {
                    format!("message must not be signed off (contain '{}')", pattern)
                }
            };
            match rule.level {
...
mod tests {
    ...
    fn test_invalid_type() {
        ...
        assert!(result
            .results
            .iter()
            .any(|r| r.name == "type-enum" && !r.valid));
    }
    ...
    fn test_empty_subject() {
        ...
        assert!(result
            .results
            .iter()
            .any(|r| r.name == "subject-empty" && !r.valid));
    }
    ...
    fn test_subject_with_full_stop() {
        ...
        assert!(result
            .results
            .iter()
            .any(|r| r.name == "subject-full-stop" && !r.valid));
    }
