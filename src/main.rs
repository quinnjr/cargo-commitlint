...
fn main() {
    ...
    let result = match cli.command {
        ...
        Some(Commands::PrintConfig { format }) => run_print_config(&format, cli.config),
        Some(Commands::Install { force }) => hook::HookInstaller::install(force)
            .map(|_| 0)
            .map_err(|e| format!("Failed to install hook: {}", e)),
        Some(Commands::Uninstall) => hook::HookInstaller::uninstall()
            .map(|_| 0)
...
fn load_config(config_path: Option<&std::path::Path>) -> Result<Config, String> {
    if let Some(path) = config_path {
        ...
        Config::from_default_locations().map_err(|e| format!("Failed to load config: {}", e))
    }
}
