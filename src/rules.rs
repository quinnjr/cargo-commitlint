...
// The `Case` suffix is load-bearing: `rename_all = "kebab-case"` turns these
// variant names into the documented config values (`lower-case`, `camel-case`,
// ...), so renaming them would silently break every user config.
    ...
    pub fn validate(&self, text: &str) -> bool {
        ...
        match self {
            ...
            CaseType::SentenceCase => {
                ...
                first
                    .map(|c| c.is_uppercase() || c.is_lowercase() || c.is_numeric())
                    .unwrap_or(true)
            }
            CaseType::SnakeCase => {
                text.chars()
    ...
    pub fn conventional() -> Self {
        Self {
            ...
            subject_case: Rule {
                level: RuleLevel::Error,
                applicable: Applicable::Always,
                value: vec![CaseType::LowerCase, CaseType::SentenceCase],
            },
            subject_empty: Rule {
                level: RuleLevel::Error,
                applicable: Applicable::Never,
